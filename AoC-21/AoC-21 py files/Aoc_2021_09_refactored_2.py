...
def part_2(lowerpoints):
    ...
    basins_size = []
    # set all points flag to False in a map copy before walking / keeping a coy of
    basinspoints_ = {pt:[False, lowerpoints[pt][1]] for pt in lowerpoints}
    for point in [pt for pt in lowerpoints if lowerpoints[pt][0]]:
        # exploring from a new map
        basinspoints = basinspoints_
...
if __name__ == '__main__':
    ...
    dt_data = {(i+1, j+1):[False, pz_data[j][i]] for i in range(maxx) for j in range(maxy)}
